...
$items.merge!($jewelry)
# This is can also be written like this: $items = $items.merge($jewelry)

def item_names
  names_of_items = []
  $items.each {|id,item_hash| names_of_items.push(item_hash["name"]) }
  names_of_items
end

def item_names
  names_of_items = []
  $items.each do |id| names_of_items << $items[id]["name"]
  end
  names_of_items
end
...
def item_margins
  margins = []
  $items.each do |calc|
    margins.push(calc[sell amount] - calc[manufacturer cost]) / calc[sell amount]
   end
end
...
def item_margins
   $items.map do |id, item|
    ...
    something = profit / item["sell amount"]
     something.round(2)
end
